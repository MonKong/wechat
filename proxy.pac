var FindProxyForURL = function(init, profiles) {
    ...
    "+SSR": function(url, host, scheme) {
        "use strict";
        if (/\.youdao\.com$/.test(host) || /\.xmcdn\.com$/.test(host) || /\.ximalaya\.com$/.test(host) || /\.xiao84\.com$/.test(host) || /\.server\.wyserver\.com$/.test(host) || /\.weiyun\.com$/.test(host) || /\.ifunmac\.com$/.test(host) || /\.erp12\.lii\.com\.cn/.test(host) || /\.portal\.lii\.com\.cn/.test(host) || /\.baidu\.com/.test(host) || /\.alicdn\.com/.test(host) || /\.wycrm\.com\.cn$/.test(host) || /tplogin\.cn$/.test(host) || /^127\.0\.0\.1$/.test(host) || /^::1$/.test(host) || /^::1$/.test(host) || /^localhost$/.test(host) || /^10\.1\..*\./.test(host) || /^103\.28\.214\.63$/.test(host)) return "DIRECT";
